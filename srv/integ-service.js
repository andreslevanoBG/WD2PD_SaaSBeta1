...
module.exports = cds.service.impl((srv) => {
  ...
  const { Integration_Pck_Planning_Adata } = srv.entities(
    "shapein.integrations"
  );
  ...
  const { Di_Template_Worker_Attr_Values } = srv.entities(
    "shapein.integrations"
  );
  ...
  const { Di_Template_Page_Content } = srv.entities("shapein.integrations");
  ...
  srv.after("READ", "Integrations", (integrations, req) => {
    try {
        ...
        if (Array.isArray(integrations)) {
          ...
          integrations.map(async (integration) => {
            if (integration.status_code == "R") {
              ...
              i = i + 1;
              integration.numberOfItems = items[0].count;
            }
            if (i == l) {
              items = null;
              resolve(integrations);
          ...
          if (integrations) {
            if (integrations.status_code == "R") {
              ...
              i = i + 1;
              integrations.numberOfItems = items2[0].count;
            }
            resolve(integrations);
          } else {
            resolve(integrations);
      ...
      /* return promise.then((integrations) => {
         if (req._query) {
          ...
          if (req._query.$filter) {
            if (req._query.$filter.includes("numberOfItems")) {
              ...
              for (let i = 0; i < integrations.length; i++) {
                ...
                if (
                  currentInteg.numberOfItems < value1 ||
                  currentInteg.numberOfItems > value2
                ) {
                  integrations.splice(i, 1);
                  i = i - 1;
                }
              }
              if (req._query.$top && req._query.$skip) {
                var top = parseInt(req._query.$top);
                var skip = parseInt(req._query.$skip);
                top = top + skip;
                var results = integrations.slice(skip, top);
  ...
  /*srv.before("READ", "Integrations", async (req) => {
    try {
      ...
      var lday;
      if (
        req._path.includes("Integrations(") ||
        (req._path.includes("Integrations/") && !req._path.includes("$count"))
      ) {
        lday = "";
      } else {
        if (query) {
          ...
          if (filters && filters.includes("timestamp")) {
           ...
           if (filters && filters.includes("numberOfItems")) {
            ...
            if (top) {
              SELECT.limit.rows.val = 999999;
              //query.$top = "999999";
            }
            if (skip) {
              SELECT.limit.offset.val = 0;
              //query.$skip = "0";
            }
          }
        } else {
  ...
  srv.after("UPDATE", "Integrations", async (req) => {
    ...
    try {
      ...
      if (status == "S") {
        var items = await cds.transaction(req).run(
          SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
            integ_id: integ_id,
          })
        );
        var num_items = items[0].count;
  ...
  srv.before("UPDATE", "Integrations", async (req) => {
    try {
      ...
      var integration = await srv.run(
        SELECT.one.from(Integrations).where({
          id: req.data.id,
        })
      );
      var date1 = new Date(req.data.timestamp_end);
      var date2 = new Date(integration.timestamp_start);
      var diff = (date1.getTime() - date2.getTime()) / 1000;
      req.data.numberOfSeconds = diff;
      ...
      req.data.numberOfItems = items[0].count;

      if (status == "S") {
        var items_S = await cds.transaction(req).run(
          SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
            integ_id: integ_id,
            status_code: "S",
          })
        );
        var num_S = items_S[0].count;

        var items_E = await cds.transaction(req).run(
          SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
            integ_id: integ_id,
            status_code: "E",
          })
        );
        var num_E = items_E[0].count;
        var pck_code = req.data.pck_code;
  ...
  srv.before("CREATE", "Integrations", async (req) => {
    try {
      ...
      req.data.numberOfItems = 0;
      req.data.numberOfSeconds = 0;
    } catch (err) {}
  });
  ...
  /*srv.before("READ", "Di_Generation_Processes", async (req) => {
    try {
      ...
      var lday;
      if (
        req._path.includes("Di_Generation_Processes(") ||
        (req._path.includes("Di_Generation_Processes/") &&
          !req._path.includes("$count"))
      ) {
        lday = "";
      } else {
        if (query) {
  ...
  srv.after("UPDATE", "Di_Template", async (req) => {
    try {
      ...
      if (!signature) {
        ...
        if (items.length > 0) {
          await srv.run(
            DELETE.from(Di_Template_Sign_Cfg).where({
              template_uuid: uuid_temp,
            })
          );
        }
      }
      if (active) {
        var template = await srv.run(
          SELECT.one.from(Di_Template).where({
            uuid: uuid_temp,
          })
        );

        var templates = await srv.run(
          SELECT.from(Di_Template).where({
        ...
        if (templates && Array.isArray(templates)) {
          for (var i = 0; i < templates.length; i++) {
            if (
              templates[i].template_version < template.template_version &&
              !templates[i].deprecated
            ) {
  ...
  srv.before("UPDATE", "Di_Template", async (req) => {
    try {
      ...
      if (req.data.doc_type_id) {
        var temp = await srv.run(
          SELECT.one.from(Di_Template).where({ uuid: req.data.uuid })
        );
        if (temp) {
          if (temp.doc_type_id != req.data.doc_type_id) {
            //change doc type
            var items = await srv.run(
            ...
            if (items.length > 0) {
              await srv.run(
                DELETE.from(Di_Template_Mappings).where({
                  template_uuid: req.data.uuid,
                  mapping_object: "DOCID_META",
                })
              );
  ...
  srv.before("UPDATE", "Di_Generation_Processes", async (req) => {
    try {
      ...
      var proc = await srv.run(
        SELECT.one.from(Di_Generation_Processes).where({
          uuid: gen_id,
        })
      );
      var date1 = new Date(req.data.timestamp_end);
      var date2 = new Date(proc.timestamp_start);
      var diff = (date1.getTime() - date2.getTime()) / 1000;
      req.data.numberOfSeconds = diff;
      var status = req.data.status_code;
      var items = await cds.transaction(req).run(
        SELECT.from(Di_Generation_Processes_Doc, ["Count(uuid) as count"]).where({
          genproc_uuid: gen_id
        })
      );
      req.data.numberOfItems = items[0].count;
      if (status == "S") {
        var items_S = await cds.transaction(req).run(
          SELECT.from(Di_Generation_Processes_Doc, [
            "Count(uuid) as count",
          ]).where({
            genproc_uuid: gen_id,
            status: "S",
        ...
        var num_E = items_E[0].count;
        /*     var items = await srv.run(
          SELECT.from(Di_Generation_Processes_Doc).where({
            genproc_uuid: gen_id,
          })
        );
        if (items.length > 0) {
  ...
  srv.after("UPDATE", "Di_Generation_Processes", async (req) => {
    try {
      ...
      var type = req.type;
      //var planning_uuid = req.planning_uuid;
      var process = await cds
        .transaction(req)
        .run(SELECT.one.from(Di_Generation_Processes).where({ uuid: gen_id }));
      if (process) {
        if (
          process.planning_uuid &&
          status != "R" &&
          process.planning_uuid != ""
        ) {
          const eventp = {
    ...
    try {
      ...
      do {
        try {
          times++;
          await srv.run(
            DELETE.from(Integrations).where({ id: msg.data.integ_id })
          );
          del = "X";
        } catch (e) {
          await sleep(500);
    ...
    try {
      ...
      do {
        try {
          times++;
          await srv.run(
            DELETE.from(Di_Generation_Processes).where({
              uuid: msg.data.gen_id,
            })
          );
          del = "X";
        } catch (e) {
  ...
  srv.on("deleteIntegItem", async (msg) => {
    try {
      srv.run(
        DELETE.from(Integration_Items).where({ item_id: msg.data.item_id })
      );
      //console.log('==> Received msg of type myEventName:' + msg.data.integ_id);
    } catch (err) {}
  });
  ...
  srv.on("updatePlanning", async (msg) => {
    try {
      srv.run(
        UPDATE(Integration_Pck_Planning)
          .set({
  ...
  srv.on("updateOrg", async (msg) => {
    try {
      ...
      var org = await srv.run(
        SELECT.from(Organizations).where({ external_id: msg.data.external_id })
      );
      if (org.length > 0) {
        ...
        await srv.run(
          UPDATE(Organizations)
            .set({
              ...
              name: msg.data.name,
              corporate_name: msg.data.corporate_name,
              last_item_id: msg.data.last_item_id,
              last_status: msg.data.last_status,
  ...
  srv.on("updateWorker", async (msg) => {
    try {
      var worker = await srv.run(
        SELECT.from(Workers).where({ external_id: msg.data.external_id })
      );
      if (worker.length > 0) {
        if (!msg.data.original_external_id) {
        ...
        await srv.run(
          UPDATE(Workers)
            .set({
              ...
              original_external_id: msg.data.original_external_id,
              employee_number: msg.data.employee_number,
              lastname: msg.data.lastname,
              email: msg.data.email,
              last_item_id: msg.data.last_item_id,
              last_status: msg.data.last_status,
              organization_id: msg.data.organization_id,
              last_timestamp: msg.data.last_timestamp,
            })
            .where({ external_id: msg.data.external_id })
        );
  ...
  srv.on("updateStatus", async (msg) => {
    try {
      if (msg.data.pck_code == "SYN_WORKER") {
        srv.run(
          UPDATE(Workers)
            .set({
              last_status: msg.data.status,
            })
            .where({ external_id: msg.data.external_id })
        );
      } else if (msg.data.pck_code == "SYN_ORG") {
        srv.run(
          UPDATE(Organizations)
            .set({
              last_status: msg.data.status,
      ...
      } else if (msg.data.pck_code == "SYN_USER") {
        srv.run(
          UPDATE(Users)
            .set({
              last_status: msg.data.status,
  ...
  srv.on("updateUser", async (msg) => {
    try {
      var user = await srv.run(
        SELECT.from(Users).where({ external_id: msg.data.external_id })
      );
      if (user.length > 0) {
        if (!msg.data.original_external_id) {
  ...
  srv.after("UPDATE", "Integration_Items", async (req) => {
    try {
      var status = req.status_code;
      var item_id = req.item_id;
      var integ_item = await srv.run(
        SELECT.one.from(Integration_Items).where({ item_id: item_id })
      );
      //var type = req.type;
      if (integ_item) {
        ...
        if (external_id && external_id != "") {
          const eventp = {
            ...
            pck_code: integ_item.pck_code,
            status: status,
          };
          srv.emit("updateStatus", eventp);
        }
      }
    } catch (err) {}
  });
  ...
  srv.before("DELETE", "Integration_Pck_Planning", async (req) => {
    try {
      var items = await cds.transaction(req).run(
        SELECT.from(Integration_Pck_Planning_D).where({
          planning_uuid: req.data.uuid,
        })
      );
      if (items.length > 0) {
        srv.run(
          DELETE.from(Integration_Pck_Planning_D).where({
            planning_uuid: req.data.uuid,
          })
        );
      }
      ...
      var itemsA = await cds.transaction(req).run(
        SELECT.from(Integration_Pck_Planning_Adata).where({
          planning_uuid: req.data.uuid,
        })
      );
      if (itemsA.length > 0) {
        srv.run(
          DELETE.from(Integration_Pck_Planning_Adata).where({
            planning_uuid: req.data.uuid,
          })
        );
      }
  ...
  srv.before("DELETE", "Di_Template_Worker_Attr", async (req) => {
    try {
      var items = await cds.transaction(req).run(
        SELECT.from(Di_Template_Worker_Attr_Values).where({
          attr_uuid: req.data.uuid,
        })
      );
      if (items.length > 0) {
        srv.run(
          DELETE.from(Di_Template_Worker_Attr_Values).where({
            attr_uuid: req.data.uuid,
          })
        );
      }
  ...
  srv.after("CREATE", "Di_Generation_Processes_Doc", async (req) => {
    try {
      ...
      if (gen_proc) {
        var emp_uuid;
        var employee = await srv.run(
          SELECT.one.from(Di_Employee).where({
        ...
        var emp_template = await srv.run(
          SELECT.one.from(Di_Employee_Template).where({
  ...
  srv.after("CREATE", "Integration_Items", async (req) => {
    try {
      ...
      var configuration = await srv.run(
        SELECT.from(Configurations).where({
          pck_code: pck_code,
      ...
      if (logic_type != "01") {
        if (pck_code == "SYN_WORKER") {
          ...
          await srv.emit("updateWorker", event);
          event = null;
        } else if (pck_code == "SYN_ORG") {
          var event = {
            external_id: external_id,
          ...
          await srv.emit("updateOrg", event);
          event = null;
        } else if (pck_code == "SYN_USER") {
          if (objectValue["registration_references"]) {
            var reg_ref = objectValue["registration_references"];
          ...
          await srv.emit("updateUser", event);
          event = null;
        }
      }
    } catch (err) {}
      ...
      const promise = new Promise(async (resolve, reject) => {
        if (Array.isArray(workers)) {
          ...
          workers.map(async (worker) => {
            ...
            i = i + 1;
            worker.numberOfItems = items[0].count;
            if (i == l) {
              resolve(workers);
              items = null;
            }
          ...
          if (workers) {
            var items2 = await cds.transaction(req).run(
              SELECT.from(Integration_Items, ["Count(item_id) as count"]).where(
                {
                  external_id: workers.external_id,
                }
              )
            );
            workers.numberOfItems = items2[0].count;
            resolve(workers);
          } else {
            resolve(workers);
          }
        ...
        if (Array.isArray(processes)) {
          ...
          processes.map(async (process) => {
            if (process.status == "R") {
              ...
              i = i + 1;
              process.numberOfItems = items[0].count;
            }
            if (i == l) {
              items = null;
              resolve(processes);
          ...
          if (processes) {
            if (processes.status == "R") {
              ...
              i = i + 1;
              processes.numberOfItems = items2[0].count;
            }
            resolve(processes);
          } else {
            resolve(processes);
      ...
      const promise = new Promise(async (resolve, reject) => {
        if (Array.isArray(users)) {
          ...
          users.map(async (user) => {
            ...
            i = i + 1;
            user.numberOfItems = items[0].count;
            if (i == l) {
              resolve(users);
              items = null;
            }
          ...
          if (users) {
            ...
            users.numberOfItems = items2[0].count;
            resolve(users);
          } else {
            resolve(users);
          }
      ...
      const promise = new Promise(async (resolve, reject) => {
        if (Array.isArray(organizations)) {
          ...
          organizations.map(async (organization) => {
            ...
            i = i + 1;
            organization.numberOfItems = items[0].count;
            if (i == l) {
              resolve(organizations);
              items = null;
            }
          ...
          if (organizations) {
            var items2 = await cds.transaction(req).run(
              SELECT.from(Integration_Items, ["Count(item_id) as count"]).where(
                {
                  external_id: organizations.external_id,
                }
              )
            );
            organizations.numberOfItems = items2[0].count;
            resolve(organizations);
          } else {
            resolve(organizations);
          }
  ...
  srv.on("delete_mappings", async (req) => {
    try {
      var future_deletes = await srv.run(
        SELECT.from(Di_Template_Mappings).where({
          template_uuid: req.data.uuid,
  ...
  srv.on("delete_attr_values", async (req) => {
    try {
      var future_va_deletes = await srv.run(
        SELECT.from(Di_Template_Worker_Attr_Values).where({
          attr_uuid: req.data.uuid,
        })
      );
  ...
  srv.on("delete_conf_signature", async (req) => {
    try {
      var future_conf_deletes = await srv.run(
        SELECT.from(Di_Template_Sign_Cfg).where({ uuid: req.data.uuid })
      );
      if (future_conf_deletes.length > 0) {
        await srv.run(
          DELETE.from(Di_Template_Sign_Cfg).where({ uuid: req.data.uuid })
        );
      ...
      if (template) {
        ...
        if (mappings.length > 0) {
          for (var i = 0; i < mappings.length; i++) {
            if (
              mappings[i].required &&
              (mappings[i].mapping == "" || mappings[i].mapping == null)
            ) {
        ...
        if (signat) {
          if (template.signature) {
            var sign = await srv.run(
              SELECT.one
                .from(Di_Template_Sign_Cfg)
                .where({ template_uuid: req.data.uuid })
            );
            if (!sign) {
  ...
  srv.on("delete_planning_repro", async (req) => {
    try {
      ...
      if (template && template.planning_rep_uuid !== null) {
        var plan_d = await srv.run(
          SELECT.from(Integration_Pck_Planning_D).where({
            planning_uuid: template.planning_rep_uuid,
          })
        );
        ...
        //        }
        await srv.run(
          DELETE.from(Integration_Pck_Planning).where({
            uuid: template.planning_rep_uuid,
          })
        );
  ...
  srv.on("delete_complete_template", async (req) => {
    try {
      var temp_del = await srv.run(
        SELECT.one.from(Di_Template).where({ uuid: req.data.uuid })
      );
      if (temp_del) {
        var pages_del = await srv.run(
          SELECT.from(Di_Template_Page_Content).where({
            template_uuid: req.data.uuid,
          })
        );
        if (pages_del && Array.isArray(pages_del)) {
          for (var i = 0; i < pages_del.length; i++) {
            await srv.run(
              DELETE.from(Di_Template_Page_Content).where({
                uuid: pages_del[i].uuid,
              })
            );
          }
        }

        var map_del = await srv.run(
          SELECT.from(Di_Template_Mappings).where({
            template_uuid: req.data.uuid,
          })
        );
        if (map_del && Array.isArray(map_del)) {
          for (var i = 0; i < map_del.length; i++) {
            await srv.run(
              DELETE.from(Di_Template_Mappings).where({ uuid: map_del[i].uuid })
            );
          }
        }

        var signConfs = await srv.run(
          SELECT.from(Di_Template_Sign_Cfg).where({
            template_uuid: req.data.uuid,
          })
        );
        ...
        if (temp_del.planning_uuid && temp_del.planning_uuid !== null) {
          var planning_del = await srv.run(
            SELECT.one
              .from(Integration_Pck_Planning)
              .where({ uuid: temp_del.planning_uuid })
          );
          if (planning_del) {
            await srv.run(
              DELETE.from(Integration_Pck_Planning).where({
                uuid: planning_del.uuid,
              })
            );
          }
          var pland_del = await srv.run(
            SELECT.one
              .from(Integration_Pck_Planning_D)
              .where({ planning_uuid: temp_del.planning_uuid })
          );
          if (pland_del) {
            await srv.run(
              DELETE.from(Integration_Pck_Planning_D).where({
          ...
          var planadata_del = await srv.run(
            SELECT.from(Integration_Pck_Planning_Adata).where({
              planning_uuid: temp_del.planning_uuid,
            })
          );
        ...
        if (temp_del.planning_rep_uuid && temp_del.planning_rep_uuid !== null) {
          var planning_rep_del = await srv.run(
            SELECT.one
              .from(Integration_Pck_Planning)
              .where({ uuid: temp_del.planning_rep_uuid })
          );
          if (planning_rep_del) {
            await srv.run(
              DELETE.from(Integration_Pck_Planning).where({
                uuid: planning_rep_del.uuid,
              })
            );
          }
          var pland_rep_del = await srv.run(
            SELECT.one
              .from(Integration_Pck_Planning_D)
              .where({ planning_uuid: temp_del.planning_rep_uuid })
          );
          if (pland_rep_del) {
            await srv.run(
              DELETE.from(Integration_Pck_Planning_D).where({
                uuid: pland_rep_del.uuid,
              })
            );
          }
        }

        var worker_attr_del = await srv.run(
          SELECT.from(Di_Template_Worker_Attr).where({
            template_uuid: req.data.uuid,
          })
        );
        if (worker_attr_del && Array.isArray(worker_attr_del)) {
          for (var i = 0; i < worker_attr_del.length; i++) {
            var attr_values_del = await srv.run(
              SELECT.from(Di_Template_Worker_Attr_Values).where({
                attr_uuid: worker_attr_del[i].attr_uuid,
              })
            );
            if (attr_values_del && Array.isArray(attr_values_del)) {
              for (var j = 0; j < attr_values_del.length; j++) {
                await srv.run(
                  DELETE.from(Di_Template_Worker_Attr_Values).where({
                    uuid: attr_values_del[j].uuid,
                  })
                );
              }
            }
            await srv.run(
              DELETE.from(Di_Template_Worker_Attr).where({
                uuid: worker_attr_del[i].uuid,
              })
            );
          }
        }
  ...
  srv.on("updateCalculatedFields", async (req) => {
    try {
      var integration = await srv.run(
        SELECT.one.from(Integrations).where({
          id: req.data.id,
      ...
      if (integration) {
        if (integration.status_code != "R") {
          var pck_code = integration.pck_code;
          var integ_id = req.data.id;
          var items = await cds.transaction(req).run(
            SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
              integ_id: integ_id,
        ...
        var num_S = items_S[0].count;*/
          var exist_S = "";
          var exist_E = "";
          var item_S = await cds.transaction(req).run(
            SELECT.one.from(Integration_Items).where({
              integ_id: integ_id,
              status_code: "S",
            })
          );
        ...
        var num_E = items_E[0].count;*/
          var item_E = await cds.transaction(req).run(
            SELECT.one.from(Integration_Items).where({
              integ_id: integ_id,
              status_code: "E",
            })
          ...
          if (exist_E == "X" && exist_S == "X") {
            ...
            status = "E";
            if (pck_code == "SYN_WORKER") {
              error_code = "W_RT_H001";
            } else if (pck_code == "SYN_ORG") {
              error_code = "O_RT_H001";
            } else if (pck_code == "SYN_USER") {
              error_code = "U_RT_H001";
            }
            // } else if (num_S > 0) {
          } else if (exist_S == "X") {
            status = "S";
          ...
          return "Integration is Running";
        }
      } else {
        return "Integration don't exist";
      }
    } catch (err) {}
  });
  ...
  srv.on("copy_template_config", async (req) => {
    try {
      ...
      var temp_ori = await srv.run(
        SELECT.one.from(Di_Template).where({ uuid: req.data.uuid_to_copy })
      );
      var temp_new = await srv.run(
        SELECT.one.from(Di_Template).where({ uuid: temp_new_uuid })
      );
      if (temp_new.active) {
      ...
      if (temp_ori && temp_new) {
        if (req.data.mappings == "true") {
          ...
          if (mappings && Array.isArray(mappings)) {
            ...
            if (mappings.length > 0) {
              await srv.run(INSERT(mappings).into(Di_Template_Mappings));
            }
          }
        }
        if (req.data.metadata == "true") {
          var meta_del = await srv.run(
            SELECT.from(Di_Template_Mappings).where({
              template_uuid: temp_new_uuid,
              mapping_object: "DOCID_META",
            })
          ...
          if (meta_del && Array.isArray(meta_del)) {
            for (var i = 0; i < meta_del.length; i++) {
              await srv.run(
                DELETE.from(Di_Template_Mappings).where({
                  uuid: meta_del[i].uuid,
                })
              );
            }
          }
          var metadatas = await srv.run(
            SELECT.from(Di_Template_Mappings).where({
              template_uuid: temp_ori_uuid,
              mapping_object: "DOCID_META",
            })
          ...
          if (metadatas && Array.isArray(metadatas)) {
            ...
            if (metadatas.length > 0) {
              await srv.run(INSERT(metadatas).into(Di_Template_Mappings));
            }
          }
        }
        if (req.data.variables == "true") {
          var var_del = await srv.run(
            SELECT.from(Di_Template_Mappings).where({
              template_uuid: temp_new_uuid,
          ...
          if (variables && Array.isArray(variables)) {
            ...
            if (variables.length > 0) {
              await srv.run(INSERT(variables).into(Di_Template_Mappings));
            }
          }
        }
        if (req.data.sign_conf == "true") {
          var signConf = await srv.run(
            SELECT.one
              .from(Di_Template_Sign_Cfg)
              .where({ template_uuid: temp_new_uuid })
          );
          if (signConf) {
            // for (var i = 0; i < signConfs.length; i++) {
            await srv.run(
              DELETE.from(Di_Template_Sign_Cfg).where({ uuid: signConf.uuid })
            );
            //  }
          }
          var sign_Config = await srv.run(
            SELECT.one
              .from(Di_Template_Sign_Cfg)
              .where({ template_uuid: temp_ori_uuid })
          );
          if (sign_Config) {
            //  if (sign_Config.length > 0) {
            sign_Config.template_uuid = temp_new_uuid;
            delete sign_Config.uuid;
            await srv.run(INSERT(sign_Config).into(Di_Template_Sign_Cfg));
            // }
          }
        }
          ...
          if (req.data.planning == "true") {
            ...
            if (temp_new.planning_uuid && temp_new.planning_uuid !== null) {
              var planning_del = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning)
                  .where({ uuid: temp_new.planning_uuid })
              );
              if (planning_del) {
                await srv.run(
                  DELETE.from(Integration_Pck_Planning).where({
              ...
              var pland_del = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning_D)
                  .where({ planning_uuid: temp_new.planning_uuid })
              );
              if (pland_del) {
                await srv.run(
                  DELETE.from(Integration_Pck_Planning_D).where({
            ...
            var planning = await srv.run(
              SELECT.one
                .from(Integration_Pck_Planning)
                .where({ uuid: temp_ori.planning_uuid })
            );
            if (planning) {
              ...
              var plan_d = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning_D)
                  .where({ planning_uuid: temp_ori.planning_uuid })
              );
              if (plan_d) {
                delete plan_d.uuid;
                plan_d.planning_uuid = new_plan.uuid;

                await srv.run(INSERT(plan_d).into(Integration_Pck_Planning_D));
              }
              var plan_adata = await srv.run(
                SELECT.from(Integration_Pck_Planning_Adata).where({
                  planning_uuid: temp_ori.planning_uuid,
                })
              );
              if (plan_adata && Array.isArray(plan_adata)) {
                for (var i = 0; i < plan_adata.length; i++) {
        ...
        if (temp_ori.planning_rep_uuid && temp_ori.planning_rep_uuid !== "") {
          if (req.data.planning_rep == "true") {
            // Borramos planning anterior si existe
            if (
              temp_new.planning_rep_uuid &&
              temp_new.planning_rep_uuid !== null
            ) {
              var planning_rep_del = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning)
                  .where({ uuid: temp_new.planning_rep_uuid })
              );
              if (planning_rep_del) {
                await srv.run(
                  DELETE.from(Integration_Pck_Planning).where({
              ...
              var pland_rep_del = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning_D)
                  .where({ planning_uuid: temp_new.planning_rep_uuid })
              );
              if (pland_rep_del) {
                await srv.run(
                  DELETE.from(Integration_Pck_Planning_D).where({
            ...
            // Copiamos la de la otra template
            var planning_rep = await srv.run(
              SELECT.one
                .from(Integration_Pck_Planning)
                .where({ uuid: temp_ori.planning_rep_uuid })
            );
            if (planning_rep) {
              ...
              var plan_d_rep = await srv.run(
                SELECT.one
                  .from(Integration_Pck_Planning_D)
                  .where({ planning_uuid: temp_ori.planning_rep_uuid })
              );
              if (plan_d_rep) {
  ...
  srv.on("set_user_future_changes", async (req) => {
    try {
      if (req.data.date == null) {
        var future_delete = await srv.run(
          SELECT.from(User_Future_Changes).where({ external_id: req.data.uuid })
        );
        if (future_delete.length > 0) {
          var delRow = await srv.run(
            DELETE.from(User_Future_Changes).where({
              external_id: req.data.uuid,
            })
          );
          if (delRow) {
        ...
        var future_change = await srv.run(
          SELECT.from(User_Future_Changes).where({ external_id: req.data.uuid })
        );
        if (future_change.length > 0) {
          var updRow = await srv.run(
            UPDATE(User_Future_Changes)
              .set({
                date: req.data.date,
              })
    ...
    try {
      ...
      if (Array.isArray(configuration) && configuration.length > 0) {
        ...
        if (ret_period && Array.isArray(ret_period)) {
          var logic_type = ret_period["0"]["logic_type"][0];
          if (logic_type == "03") {
            var days_complete_employee = parseInt(
              ret_period["0"]["complete_employee_data"][0]
            );
            var days_employee_details = parseInt(
              ret_period["0"]["employee_details"][0]
            );
            ...
            if (days_employee_details >= 1) {
              ...
              var limit =
                ...
                seconds +
                "Z";
              var workeritems = await srv.run(
                SELECT.from(Integration_Items).where({
                  pck_code: "SYN_WORKER",
                  // request_deleted: { '<>': 'X' },
            ...
            if (days_complete_employee >= 1) {
              ...
              var limit3 =
                ...
                seconds3 +
                "Z";
              var integrations = await srv.run(
                SELECT.from(Integrations).where({
                  pck_code: "SYN_WORKER",
                  createdAt: { "<=": limit3 },
                })
              );
  ...
  srv.on("di_business_process_type_master_delete_all", async (req) => {
    try {
      ...
      await srv.run(DELETE.from(Di_Business_Process_Master));
    } catch (err) {}
  ...
  srv.on("counterIntWorkers", async (req) => {
    var date = new Date();
    date.setDate(date.getUTCDate() - 1);
    var day =
    ...
    var items = await cds.transaction(req).run(
      SELECT.from(Integrations, ["Count(id) as count"]).where({
        pck_code: "SYN_WORKER",
        timestamp_start: { ">=": lastday },
      })
    );
    var result = {};
    result.number = items[0].count;
    return result;
  });

  srv.on("counterIntOrgs", async (req) => {
    var date = new Date();
    date.setDate(date.getUTCDate() - 1);
    var day =
        date.getUTCDate() < 10 ? "0" + date.getUTCDate() : date.getUTCDate(),
  ...
  srv.on("counterIntUsers", async (req) => {
    var date = new Date();
    var whereaux = [];
    date.setDate(date.getUTCDate() - 1);
    var day =
  ...
  srv.on("getCountersInteg", async (req) => {
    var itemsS = await cds.transaction(req).run(
      SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
        integ_id: req.data.id,
        status_code: "S",
      })
    );
    var result = {};
    result.countS = itemsS[0].count;

    var itemsE = await cds.transaction(req).run(
      SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
        integ_id: req.data.id,
        status_code: "E",
      })
    );
    result.countE = itemsE[0].count;

    var itemsD = await cds.transaction(req).run(
      SELECT.from(Integration_Items, ["Count(item_id) as count"]).where({
        integ_id: req.data.id,
        status_code: "D",
      })
    );
    result.countD = itemsD[0].count;
    return result;
  });

  srv.on("counterIntProc", async (req) => {
    var date = new Date();
    var whereaux = [];
    date.setDate(date.getUTCDate() - 1);
    var day =
    ...
    var items = await cds.transaction(req).run(
      SELECT.from(Di_Generation_Processes, ["Count(uuid) as count"]).where({
        timestamp_start: { ">=": lastday },
      })
    );
    var result = {};
  ...
  srv.on("get_pages", async (req) => {
    try {
      var pages = await srv.run(
        SELECT.from(Di_Template_Page_Content)
          .where({
            template_uuid: req.data.uuidtemp,
          })
          .orderBy([{ ref: ["page"], sort: "asc" }])
      );
      if (pages && Array.isArray(pages)) {
        ...
        for (var i = 0; i < pages.length; i++) {
          var size = Buffer.byteLength(pages[i].content, "UTF8");
          total = total + size;
          if (pack == req.data.pack) {
            var item = {};
          ...
          if (total > 40000000) {
            total = 0;
            pack++;
            if (i < pages.length - 1 && pack > req.data.pack) {
              mas_env = true;
            }
            if (pack > req.data.pack) {
              break;
        ...
        var item_i = {};
        if (mas_env) {
          item_i.next_pack = pack;
        } else {
          item_i.next_pack = 0;
        }
        item_i.uuidtemp = req.data.uuidtemp;
        item_i.pages = pages_env;
        pages = null;
        return item_i;
      } else {
        var item_i = {};
  ...
  srv.on("get_organization_types", async (req) => {
    try {
      var configuration = await srv.run(
        SELECT.one.from(Configurations).where({
          pck_code: "SYN_ORG",
          conf_code: "SCE-CONFIG",
        })
      );
            ...
            if (ws_org) {
              ...
              if (req_filter) {
                ...
                if (req.data.text == "true" || req.data.metadata == "true") {
                  var typeTxt = await srv.run(
                    SELECT.one.from(Organization_Types).where({
                      code: typeOrg,
                    })
                  );
                  if (req.data.text == "true" && req.data.metadata == "true") {
