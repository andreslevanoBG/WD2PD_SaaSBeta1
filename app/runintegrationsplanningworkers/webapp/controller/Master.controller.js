sap.ui.define([
	...
	return BaseController.extend("shapein.RunIntegrationPlanningWorkers.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			this._oListFilterState.aFilter = [new Filter("pck_code", FilterOperator.EQ, "SYN_WORKER")];
		...
		onOpenViewSettings: function (oEvent) {
			...
			if (!this.byId("viewSettingsDialog")) {
				Fragment.load({
					...
					name: "shapein.RunIntegrationPlanningWorkers.view.ViewSettingsDialog",
					controller: this
			...
			var dialog = new sap.m.Dialog({
				...
				beginButton: new sap.m.Button({
					...
					press: function () {
						...
						var workerst = that.byId("multiInput2").getTokens();
						var supervisories = [];
						var workers = [];
						var refIdWorker = that.byId("RefIdWorkers").getSelectedKey();
						for (var i = 0; i < supervisoriest.length; i++) {
							var supervisor = supervisoriest[i].getText();
							supervisories.push(supervisor);
						}
						for (var i = 0; i < workerst.length; i++) {
							var worker = workerst[i].getText();
							workers.push(worker);
						}
		...
		execNewPlan: function (oEvent) {
			...
			var text = "Are you sure you want to create a Workers Planning?";
			var that = this;
		...
		createDataBatch: function (planUuid, processing_type) {
			...
			oData6.level1 = "SUPERVISORIES_ALLOWED";
			...
			var supervisoriest = this.byId("multiInput11").getTokens();
			for (var i = 0; i < supervisoriest.length; i++) {
				var oData7 = {};
		...
		newOnDemand: function (oEvent) {
			...
			if (!this.byId("ondemand")) {
				Fragment.load({
					id: this.getView().getId(),
					name: "shapein.RunIntegrationPlanningWorkers.view.fragment.OnDemand",
					controller: this
				}).then(function (oDialog) {
					// connect dialog to the root view of this component (models, lifecycle)
					this.getView().addDependent(oDialog);
