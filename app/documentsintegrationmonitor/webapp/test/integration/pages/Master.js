		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <bpt_id>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("Di_Generation_Processes");
							...
							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].uuid;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].uuid;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Di_Generation_Processes", {
								uuid: sObjectNotInTheListId
							});
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						title: oBindingContext.getProperty("bpt_id")
					};
				}
			},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "text",
							value: "<Di_Generation_Processes> (0)"
						}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("bpt_id");
				},
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("Di_Generation_Processes");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<Di_Generation_Processes> (" + iExpectedLength + ")"
								}),
								success: function () {
